...
**Recommended utility from `Init.lua`:**
```lua
...
The template ships with `ns.Debounce()` (see `Init.lua`):
```lua
...
2. **Always read the template files first** — they are the gold standard. Read `Init.lua`, `Core.lua`, and `Config.lua` for the latest patterns before generating code.
...
### Gold Standard: Project Root

The template files at the project root are the **primary source of truth** for patterns. Always read them before generating code:
- `Init.lua` — namespace, defaults, Secret Values helpers, Debounce, event dispatch, SavedVariables migration, slash commands, compartment
- `Core.lua` — combat queue, feature logic, event handlers
- `Config.lua` — modern Settings API registration
- `CLAUDE.md` — AI coding instructions for generated addons
